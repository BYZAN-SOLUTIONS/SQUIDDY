...
contract DaiStrategy{
    ...
    // squid only function for creating additional rewards from dust
    function withdraw(IERC20 _asset) external returns (uint256 balance) {
        require(msg.sender == squid, "!squid");
        require(want != address(_asset), "want");
        require(cDAI != address(_asset), "cDAI");
        require(comp != address(_asset), "comp");
        balance = _asset.balanceOf(address(this));
        _asset.transfer(squid, balance);
    }
    ...
    function withdraw(uint256 _amount) external {
        ...
        IERC20(want).transfer(ISquid(squid).rewards(), _fee);
        address _vault = ISquid(squid).vaults(address(want));
    ...
    function harvest() public {
        require( msg.sender == vaultManager, "!authorized");
    ...
    function setsquid(address _squid) external {
        require(msg.sender == vaultManager, "!vaultManager");
        squid = _squid;
    }
}